...
#        print("cts_loss:", cts_loss, e_cts_loss, ui_cts_loss)
    ...
    def get_rhs(self, t, mode):

        if mode:
            return self.entity_embedding.weight, self.bt.weight
        else:
            return self.entity_embedding(t), self.bt(t)
        
    def get_queries(self, h,r):
        
        c = F.softplus(self.c[r]) #
        head= self.entity_embedding(h) #
        rot_mat, ref_mat = torch.chunk(self.rel_diag(r), 2, dim=1)  #
        rot_q = givens_rotations(rot_mat, head).view((-1, 1, self.embedding_size))
        ref_q = givens_reflection(ref_mat, head).view((-1, 1, self.embedding_size))
        cands = torch.cat([ref_q, rot_q], dim=1) # 
       
        context_vec = self.context_vec(r).view((-1, 1, self.embedding_size)).to(self.device) #
    ...
    def calculate_kg_loss(self, interaction):
        ...
        lhs_e, lhs_biases = self.get_queries(h,r) #查询

        rhs_e, rhs_biases = self.get_rhs(pos_t,self.mode) #目标项
        pos_score = score((lhs_e, lhs_biases), (rhs_e, rhs_biases),self.mode) #打分
        positive_score = F.logsigmoid(pos_score)
        

        rhs_e, rhs_biases = self.get_rhs(neg_t,self.mode) #目标项
        neg_score = score((lhs_e, lhs_biases), (rhs_e, rhs_biases), self.mode) #打分
